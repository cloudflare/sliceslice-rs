...
use crate::{bits, memcmp::memcmp, MemchrSearcher, Needle};
use paste::paste;
use seq_macro::seq;
#[cfg(target_arch = "x86")]
use std::arch::x86::*;
...
pub struct Avx2Searcher<N: Needle + ?Sized> {
    position: usize,
...
impl<N: Needle + ?Sized> Avx2Searcher<N> {
    ...
    where
        N: Sized,
    {
        let position = needle.as_ref().len() - 1;
    ...
    where
        N: Sized,
    {
        assert!(!needle.as_ref().is_empty());
        assert!(position < needle.as_ref().len());

        let scalar_hash = ScalarHash::from(needle.as_ref());
        let sse2_hash = VectorHash::new(needle.as_ref()[0], needle.as_ref()[position]);
        let avx2_hash = VectorHash::new(needle.as_ref()[0], needle.as_ref()[position]);
    ...
    fn scalar_search_in(&self, haystack: &[u8]) -> bool {
        debug_assert!(haystack.len() >= self.needle.as_ref().len());

        let mut end = self.needle.as_ref().len() - 1;
        ...
        while end < haystack.len() {
            ...
            let start = end - self.needle.as_ref().len();
            if hash == self.scalar_hash && &haystack[start..end] == self.needle.as_ref() {
                return true;
            }
    ...
    unsafe fn vector_search_in_chunk<V: Vector>(
        ...
        let needle = self.needle.as_ref().as_ptr().add(1);

        while eq != 0 {
            let chunk = chunk.add(eq.trailing_zeros() as usize);
            if memcmp::<N>(chunk, needle, self.needle.as_ref().len() - 1) {
                return true;
            }
    ...
    unsafe fn vector_search_in<V: Vector>(
        ...
        let end = haystack.len() - self.needle.as_ref().len() + 1;
    ...
    pub unsafe fn inlined_search_in(&self, haystack: &[u8]) -> bool {
        if haystack.len() < self.needle.as_ref().len() {
            return false;
        }
...
impl DynamicAvx2Searcher {
    ...
    pub unsafe fn with_position(needle: Box<[u8]>, position: usize) -> Self {
        ...
        if let [needle] = *needle {
            return Self::N1(MemchrSearcher::new(needle));
        }

        seq!(M in 2..=13 {
            paste! {
                if needle.len() == M {
                    return Self::[<N M>](Avx2Searcher::with_position(
                        needle.as_ref().try_into().unwrap(),
                        position,
                    ));
                }
            }
        ...
        Self::N(Avx2Searcher::with_position(needle, position))
    }
